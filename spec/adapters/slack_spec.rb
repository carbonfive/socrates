...
RSpec.describe Socrates::Adapters::Slack do
  ...
  describe "#users_list" do
    ...
    it "removes deleted users and bots by default" do
      expect(adapter.users_list).to contain_exactly(*active_members)
    end

    it "includes deleted users when include_deleted is true" do
      expect(adapter.users_list(include_deleted: true)).to contain_exactly(*(active_members + deleted_members))
    end

    it "includes bots when include_bots is true" do
      expect(adapter.users_list(include_bots: true)).to contain_exactly(*(active_members + bots))
    end
  end
